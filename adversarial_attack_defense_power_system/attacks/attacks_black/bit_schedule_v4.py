...
def bit_schedule_attack_v4(model, input_image, input_label, attack_config):
    """
        This version weighted the each query in the iteration to predict the gradient.
    """
    ...
    # lr = 0.05 if epsilon_l2 == 20 else 0.01
    lr = 0.05
    # Start the attack
    print(f"Starting Bit Schedule Attack V4, with max_queries: {max_queries}, epsilon_l2: {epsilon_l2}, learning_rate: {lr}.")
    setup_random_seeds()
    # init the variables
    ...
    for iter in range(max_queries):
        top_portion = 0.5 - (0.5 - 0.1) * query_cnt / max_queries
        ...
        if abs(cur_prob - last_prob) < 1e-5 and fd_eta < epsilon_l2 / 20:
            fd_eta *= 2
            print(fd_eta)
