...
def bit_schedule_attack_transfer_v2(model, input_image, input_label, attack_config):
    """
    ...
    print(f"Starting Bit Schedule Attack transfer V2, with max_queries: {max_queries}, epsilon_l2: {epsilon_l2}, learning_rate: {lr}.")
    setup_random_seeds()
    # init the variables
    ...
    for iter in range(max_queries):
        ...
        for inner_iter in range(T):
            ...
            for bit in range(bit_length):
                ...
                # The weight is (first_prob - best_prob)
                gradient_est -= exp_noise * (first_prob - last_prob)
                query_cnt += 1
        ...
        # gradient_est = gradient_est + focus * 0.01 * gradient_est_surrogate
