...
def carlini_wagner_l2_attack(sample_x, sample_y, net, attack_config=None):
    ...
    y = sample_y.max(1, keepdim=True)[1]
    clip_min, clip_max = -20, 20
    # clip_min = torch.min(sample_x).item()
    # clip_max = torch.max(sample_x).item()

    # Perform attack
    sample_x_adv = carlini_wagner_l2(net, sample_x, 4, targeted=False, y=y, clip_min=clip_min, clip_max=clip_max)
